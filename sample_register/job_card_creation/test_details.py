...
def get_test_details():
		...
		frappe.msgprint("inside get test details")
		test_details_list = frappe.db.sql(""" select test_name,test_group from tabTest where test_group=%s;""",
				test_group)
		frappe.msgprint(test_details_list)
...
def validate_role(self, method):
		...
		role_list = frappe.db.sql("""select name,owner,parent,idx,role from tabUserRole where parent=%s and role='Blogger'""",
				user)
		if len(role_list) > 0:
			frappe.msgprint("blogger is present")
